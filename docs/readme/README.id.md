<div align="center">
  ...
  <p><a href="https://termigo.app">Situs web</a> · <a href="https://termigo.app/docs">Dokumentasi</a> · <a href="https://github.com/crynta/Termigo-website">Kode sumber situs web</a></p>

  <p>
    <img src="https://img.shields.io/github/v/release/crynta/termigo-ai?label=version&color=blue" alt="versi" />
    <img src="https://img.shields.io/github/downloads/crynta/termigo-ai/total?label=downloads&color=blue" alt="unduhan" />
...
- **Alur kerja berbasis agen:** rencana, subagen, memori proyek melalui `TERMIGO.md`, baca / tulis / sunting / multisunting / grep / glob, bash dengan persetujuan, dan proses latar belakang
...
Penginstal terbaru tersedia di halaman [Releases](https://github.com/crynta/termigo-ai/releases/latest). Termigo melakukan pembaruan otomatis dari sana.
...
- **Arch / AUR:** `yay -S termigo-bin` atau `paru`. Paket mengikuti rilis terbaru.
- **NixOS / Nix:** gunakan flake resmi. Di luar NixOS, jalankan `nix profile install github:crynta/termigo-ai`. Di NixOS, impor flake dan tambahkan `inputs.termigo.packages.${pkgs.system}.termigo` ke `environment.systemPackages`. `nixosModules.termigo` juga tersedia untuk pengaturan yang lebih sederhana.
- **AppImage:** memerlukan FUSE. Tanpanya, jalankan `./Termigo_*.AppImage --appimage-extract-and-run`. Jika ada masalah perenderan di Wayland, coba `WEBKIT_DISABLE_DMABUF_RENDERER=1`. Paket `.deb` / `.rpm` menggunakan stack GTK sistem dan biasanya lebih lancar.

## Konfigurasi AI

1. Buka **Pengaturan -> AI**.
2. Pilih penyedia dan tempel kunci API. Untuk inferensi lokal, arahkan Termigo ke endpoint LM Studio / MLX / Ollama.
...
Termigo dilisensikan di bawah Apache-2.0. Untuk informasi tentang dependensi, lihat [Apache License 2.0](../../LICENSE).
