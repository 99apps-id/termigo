<div align="center">
  ...
  <p>
    ...
    <a href="https://termigo.app/docs">Dokumentation</a> ·
    <a href="https://github.com/crynta/Termigo-website">Quellcode der Website</a>
  </p>
...
- **Agentischer Workflow:** Pläne, Sub-Agenten, Projektgedächtnis über `TERMIGO.md`, Lesen / Schreiben / Bearbeiten / Mehrfachbearbeitung / grep / glob, Bash mit Freigabe und Hintergrundprozesse
...
Die neuesten Installationspakete stehen auf der Seite [Releases](https://github.com/crynta/termigo-ai/releases/latest). Termigo aktualisiert sich von dort automatisch.
...
- **Arch / AUR:** `yay -S termigo-bin` (oder `paru` usw.). Folgt der neuesten Version.
- **NixOS / Nix:** Nutze den offiziellen Flake: `nix profile install github:crynta/termigo-ai` außerhalb von NixOS. Unter NixOS importierst du den Flake und fügst `inputs.termigo.packages.${pkgs.system}.termigo` zu `environment.systemPackages` hinzu. Für eine einfachere Einrichtung ist auch `nixosModules.termigo` verfügbar.
- **AppImage:** Benötigt FUSE. Ohne FUSE: `./Termigo_*.AppImage --appimage-extract-and-run`. Bei Darstellungsfehlern unter Wayland hilft möglicherweise `WEBKIT_DISABLE_DMABUF_RENDERER=1`. Die `.deb`- / `.rpm`-Pakete binden stattdessen den GTK-Stack des Systems ein und laufen meist flüssiger.

## KI konfigurieren

1. Öffne **Einstellungen -> KI**.
2. Wähle einen Anbieter und füge deinen API-Schlüssel ein. Für lokale Inferenz verweist du Termigo auf deinen LM Studio- / MLX- / Ollama-Endpunkt.
...
Termigo steht unter der Apache-2.0-Lizenz. Weitere Informationen zu unseren Abhängigkeiten findest du in der [Apache License 2.0](../../LICENSE).
