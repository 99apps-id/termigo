...
- **CWD and dependency traversal authorization**: the execution working directory must reside within an authorized workspace root from `WorkspaceRegistry`. Reparse points and symlinks within `node_modules` (including `.pnpm` virtual store and the pnpm global store) are authorized for reading and traversal in `is_authorized_canonical` and `authorize_spawn_cwd` when the link origin resides within an authorized workspace root. Windows drive and path comparisons are normalized case-insensitively to prevent false rejections.
- **Command risk classification (`commandRisk.ts`)**: Read-only lint and verification subcommands (`biome check`, `biome lint`, `lint`) are classified as introspection (`read_only`). Active test execution (`vitest run`, `cargo test`, `pytest`, `go test`) is classified as `change` because running arbitrary test suites executes project code and may generate snapshots or disk artifacts.
