...
- **Command allowlist (bare names)**: `validate_shell_command` checks the program of EVERY segment (`;`, `&&`, `||`, `|`, and newlines all split) against `SANDBOX_ALLOWLIST` (which includes `biome`, `lint`, `pnpm`, `vitest`, `tsc`, `pytest`, `cargo`, etc.). **Rooted/absolute paths and paths under `node_modules`/`.pnpm` are allowed by design** — the agent legitimately runs binaries it built (`./target/debug/mytool`) and local dependency tools (`./node_modules/.bin/vitest`, `node_modules/vitest/vitest.mjs`) — so the allowlist is a friction control for bare names, not a hard boundary (F-1 of the 2026-09-23 deep audit, kept as an operator decision). The hard guards are the approval flow, the delete gate below, the secret write-target refusal, and the deny-lists.
...
- **PATH for project tooling**: the spawned shell's PATH is prepended with the `node_modules/.bin` chain from the cwd upward (npm-run semantics), so `biome`/`vitest`/`tsc` resolve by bare name; on Windows the oneshot shell executes with `-ExecutionPolicy Bypass` so PowerShell `.ps1` shims execute without restriction; package-manager MUTATIONS get a 300s timeout floor because a killed install corrupts the tree.
- **CWD and dependency traversal authorization**: the execution working directory must reside within an authorized workspace root from `WorkspaceRegistry`. Reparse points and symlinks within `node_modules` (including `.pnpm` virtual store and pnpm global store) are authorized for reading and traversal when originating from an authorized workspace root.
