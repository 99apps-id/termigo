...
`src/modules/ai/agents/registry.ts` defines the sub-agent types: `explore`, `code-review`, `security`, `general`, `builder`, and `pentest`. They differ only in system prompt, not in what they may touch: every type gets the SAME toolset as the main agent (`buildTools` + extension tools), so a sub-agent is a peer that can edit, run commands and drive extensions, not a read-only subset. What keeps that safe is approval, not restriction: `runSubagent` routes every mutating, exec or third-party tool through the user's approval queue (`subagentToolNeedsGate`), exactly as the main agent gates them, so nothing runs without a click.

Sub-agents can nest up to `effectiveSubagentMaxDepth()` (default 3, preference clamped 1-5); only at the depth cap are the spawn tools (`run_subagent` / `run_subagents`) withheld (`spawnToolsWithheld`) to prevent unbounded recursion.

### Concurrency pool and deadlock avoidance

Parallel subagent execution is managed by a centralized concurrency pool:

- **`lib/subagentPool.ts` (`SubagentConcurrencyPool`)**: caps active concurrent subagents across the entire application (default limit 4) to protect against provider rate limits and local system overload.
- **Hierarchical slot yielding (`ctx.yieldSlot`)**: when a parent subagent invokes a child subagent or a batch of subagents, it yields its slot in the pool while awaiting the children. This eliminates hierarchical deadlocks where parent subagents fill the pool and starve child subagents.
- **Nested batch concurrency**: nested `run_subagents` calls bound their internal concurrency to a maximum of 2 workers to prevent combinatorial concurrency explosion.
- **Workspace isolation**: local workspace subagents isolate their workspace root from remote SSH tabs (`getRemoteSession = () => null`, `getCwd = () => workspaceRoot`), ensuring file and shell operations run strictly against the local project rather than leaking into an active remote shell.
- **Transient provider backoff**: when encountering provider concurrency limits (`concurrency reached`, `rate_limit`, HTTP 429), `runSubagent` executes an exponential backoff retry loop (up to 2 retries) before failing.
...
### Tool call repair and parameter normalization (`lib/repairToolCall.ts`)

Different model providers and tool ecosystems sometimes emit alternative parameter names or legacy tool identifiers. Termigo automatically normalizes tool calls before validation:

- **Parameter Aliasing**:
  - `grep`: maps `query` to `pattern`, and `path` to `root`.
  - `glob`: maps `query` to `pattern`, and `path` to `root`.
  - `code_search`: schema accepts up to 100 results, auto-clamping values to safe maximums.
  - `replace_in_files`: coerces single string `glob` values into arrays.
- **Tool Mapping**:
  - Automatically redirects equivalent signatures: `view_file` -> `read_file`, `run_command` -> `bash_run`, and `replace_file_content` -> `edit`.
  - Maps near-miss tool names using `toolNames.ts` fuzzy matching so minor hallucinations do not fail the step.
...
- **Edit & resend (run truncation).** `lib/messageEdit.ts` (`splitForEdit`) allows editing any previous user turn. The transcript is cleanly partitioned: messages up to the target turn are preserved, subsequent assistant runs and tool executions are truncated, and turn checkpoints after that point are pruned. The target text is reloaded into the composer so the user can amend their instruction and re-run without accumulating stale or contradictory conversation history.
- **Per-turn checkpoints & rewind.** `store/turnCheckpointStore.ts` captures the git working tree state before each user turn executes. Only tracked changes are captured (via `git write-tree` / `git commit-tree` with `-u` staging) to prevent touching scratch or untracked files. Checkpoints persist to `termigo-turn-checkpoints.json`. In `components/AiChat.tsx`, turns with checkpoints surface a rewind button (`ArrowTurnBackwardIcon`): clicking it rolls back the git working tree to that checkpoint's tree SHA (saving a safety recovery commit first) and prunes subsequent chat turns.
- **Session forking.** `store/chatStore.ts` (`forkSession`) allows branching a conversation from any message (user or assistant) into a brand new, independent session titled `Fork · <Label>`. The existing session and working tree are preserved untouched. The new session inherits conversation history up to the selected message, copies associated turn checkpoints, and clears session-scoped tool grants so security approvals are re-evaluated in the new branch.
- **Timeline navigator.** `components/ChatTimelineNavigator.tsx` renders a collapsible floating mini-map rail on the right side of the chat transcript whenever the transcript contains 200+ parts or 2+ user turns. Each waypoint displays turn numbers, concise message labels, checkpoint availability indicators, and part counts. Clicking a waypoint smoothly scrolls to the target turn. The active waypoint tracks viewport position in real-time via `IntersectionObserver`.
- **Permanent auto-all indicator & mutating tool diffs.** When `agentApprovalMode === "all"` is active, a persistent pulsating badge appears in the mini-window header (`components/AiMiniWindow.tsx`) with a popover to instantly revert to "ask", paired with an inline warning banner above the composer input (`components/AiComposerInput.tsx`). In `components/AiToolApproval.tsx`, mutating file tool cards (`write_file`, `edit`) provide inline collapsible diff previews with green/red line counters (+N / -N) so changes can be inspected before approval.
- **Inline ANSI-aware output & peek-to-terminal.** `components/AnsiOutput.tsx` provides a zero-dependency ANSI escape sequence parser supporting 3/4-bit standard/bright colors, 8-bit 256 colors, 24-bit RGB truecolor, and text styling attributes (bold, dim, italic, underline), mapped to `--terminal-ansi-*` theme variables. Both `BashRunOutput` and tool approval cards feature a "Peek to terminal" button that directly pastes the command into the active terminal PTY for manual inspection.
- **Lifecycle & re-render stability.** To guarantee zero re-render thrashing and avoid React error #185 during session switching or timeline scrolling, `AiChatBody` mounts with `key={sessionId}` to ensure clean component remounts; `ChatTimelineNavigator` uses stable `EMPTY_CHECKPOINTS` array references to prevent selector re-evaluation; and `IntersectionObserver` updates are guarded with functional state updaters.
