...
- **ssh/** sessions as tabs, host-key TOFU, SFTP explorer, forwarding - **extensions/** manifest + sandbox, runs untrusted code in a worker - **agents/** launching and notifications for coding agents - **ai/** the agent itself, below
...
- **Agent memory path**: learned memory is always read from `.termigo/memory.md` in the active workspace root. The workspace root comes from the explorer root or launch cwd, never from the user's home directory. If `getWorkspaceRoot()` falls back to home, the agent will try to read `C:\Users\Iwan\.termigo\memory.md` instead of the project's `.termigo/memory.md`.
