...
- **Shell sandbox allowlist** (`src-tauri/src/modules/shell/mod.rs`): package managers (`apt`, `apt-get`, `dpkg`, `pacman`, `dnf`, `brew`, `pip`, `uv`), elevation wrappers (`sudo`, `doas`, unwrapping target command), and Windows shells (`cmd`, `powershell`, `pwsh`, `set`) are allowlisted with argument sanitization and risk-checking.
