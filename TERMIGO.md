...
- **Tools & Repair** (`tools/tools.ts`, `lib/repairToolCall.ts`): inspection tools auto-execute; mutating tools require approval. `lib/security.ts` denies secret paths (`.env*`, `.ssh/`, credentials) on read and write, mirrored by `src-tauri/src/modules/fs/security.rs` (the two lists must stay in parity — tests pin both). Operator decision (2026-09-23): Windows system directories (`Windows`, `Program Files`, `ProgramData`) are OPEN to the agent — the destruction guardrail lives in the system prompt's Filesystem safety section plus the approval layer, while credential stores, secret basenames, Unix system prefixes, and `.termigo/hooks.json` + `approvals.json` (silent-exec persistence) stay hard-denied. Tool calls automatically repair parameter aliases (`query`/`path` on `grep`/`glob`, `max_results` clamped on `code_search`, string-to-array on `replace_in_files`) and near-miss tool names.
- **Shell sandbox allowlist** (`src-tauri/src/modules/shell/mod.rs`): bare program names must match `SANDBOX_ALLOWLIST` — package managers (`apt`, `dnf`, `brew`, `pip`, `uv`, `pnpm`, `npm`), project toolchains (`biome`, `tsc`, `vitest`, `eslint`, `pytest`, `ruff`, `cargo`, `go`), pentest tooling (`nmap`, `nuclei`, `sqlmap`, …), elevation wrappers (`sudo`, `doas` — target command unwrapped and checked), file utilities (`rmdir`, `mkdir`, `cp`, `mv`, `touch`; **`rm` is deliberately NOT allowlisted** — deletes go through `del`/`Remove-Item`, the approval-gated `delete_file`, or a PTY), and Windows shells/cmdlets. Rooted/absolute paths are allowed by design (the agent runs binaries it built), so the list is a friction control for bare names, not a hard boundary — the effective guards are the approval layer (deletes always ask), destructive-command refusals (`rm -rf /`/`~`), the secret write-target check (`Set-Content .env` refused like `write_file` refuses it), hijack-env-var refusals (`PATH=`, `NODE_OPTIONS=--require`, PowerShell `$env:` forms), and the system prompt's Filesystem safety section. The agent shell's PATH auto-includes the project's `node_modules/.bin` chain, and package-manager mutations get a 300s timeout floor so a killed install cannot corrupt the tree.
