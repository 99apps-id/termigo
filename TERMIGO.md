...
- **Shell sandbox allowlist** (`src-tauri/src/modules/shell/mod.rs`): bare program names must match `SANDBOX_ALLOWLIST`; rooted paths are allowed by design — friction control, not a boundary. Hard guards: approval flow, delete gate, secret write-target refusal, hijack-env-var refusal. `rm` is not allowlisted. Agent-shell PATH includes `node_modules/.bin`; package-manager mutations get a 300s floor. Details: [security model](docs/architecture/security-model.md).
