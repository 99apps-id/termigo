...
- **Shell sandbox allowlist** (`src-tauri/src/modules/shell/mod.rs`): package managers (`apt`, `apt-get`, `dpkg`, `pacman`, `dnf`, `brew`, `pip`, `uv`), elevation wrappers (`sudo`, `doas`, unwrapping target command), core file utilities (`rm`, `rmdir`, `mkdir`, `cp`, `mv`, `touch`, `chmod`, `chown`, `del`), and Windows shells/cmdlets (`cmd`, `powershell`, `pwsh`, `set`, `Start-Sleep`) are allowlisted with argument sanitization and risk-checking.
