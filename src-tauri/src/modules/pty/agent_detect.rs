...
mod tests {
    ...
    fn termigo_marker_drives_status() {
        ...
        assert_eq!(
            run(&mut d, &osc("777;notify;Termigo;working")),
            vec![Transition::Working]
        );
        assert!(run(&mut d, &osc("777;notify;Termigo;working")).is_empty());
        assert_eq!(
            run(&mut d, &osc("777;notify;Termigo;finished")),
    ...
    fn four_field_marker_self_arms_named_agent() {
        ...
        let mut d = AgentDetector::new();
        assert_eq!(
            run(&mut d, &osc("777;notify;Termigo;codex;working")),
            vec![started("codex")]
        );
    ...
    fn four_field_marker_drives_status_after_preexec() {
        ...
        assert_eq!(
            run(&mut d, &osc("777;notify;Termigo;gemini;attention")),
            vec![Transition::Attention]
        );
        assert_eq!(
            run(&mut d, &osc("777;notify;Termigo;gemini;working")),
            vec![Transition::Working]
        );
        assert_eq!(
            run(&mut d, &osc("777;notify;Termigo;gemini;finished")),
            vec![Transition::Finished]
        );
    }
