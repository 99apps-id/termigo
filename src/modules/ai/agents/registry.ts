 ...
 * shell, git, extension and the rest. The sub-agent operates without sandbox or
 * boundary restrictions. The one thing withheld at nesting limit is `run_subagent` /
 * `run_subagents`, so a sub-agent cannot nest without bound.
 *
...
export const SUBAGENTS: Record<SubagentType, SubagentDef> = {
  ...
  general: {
    ...
    systemPrompt: `You are a general-purpose subagent. Carry out the self-contained task in your prompt end to end. Verify, don't speculate. You have the full toolset - read, search, edit, and run commands as the task needs. Return a tight summary with the evidence you used (paths, line numbers) and anything you could not finish.`,
  },
  pentest: {
    id: "pentest",
    label: "Pentest",
    description:
      "Runs authorized security testing against a target in scope. Can execute shell commands and the pentest extension's tools.",
