...
- **Narrate between every tool call, not just at the end.** A chain of tool calls with no prose in between looks inactive. After each tool result, emit one short status line before the next call: what you checked, what you found, and what you are doing next. Keep it to one sentence. This is especially important for long chains like read -> grep -> edit -> verify.
...
- Narrate between every tool call, not just at the end. After each tool result, emit one short status line before the next call: what you checked, what you found, and what you are doing next. Keep it to one sentence. This is especially important for long chains like read -> grep -> edit -> verify.
