...
export function buildShellTools(ctx: ToolContext) {
  return {
    bash_run: tool({
      description:
        "Run a foreground shell command. When the active terminal is an SSH session the command runs ON THE REMOTE HOST, from the remote shell's working directory, and always asks for approval regardless of the approval mode. Otherwise it runs in this session's persistent local shell, where cwd persists across calls. Use for short-lived commands (build, install, service restarts, a quick grep). Package managers (`apt`, `apt-get`, `dpkg`, `pacman`, `dnf`, `yum`, `apk`, `zypper`, `brew`, `pip`, `pip3`, `pipx`, `uv`, `winget`, `choco`) and privilege elevation / root access (`sudo`, `doas`, `su`, `wsl`) are allowlisted and supported on Linux/WSL, macOS, and Windows with user approval. For project-wide lint/test use `run_checks` instead (it defaults to 300s). For long-running local daemons use `bash_background`. NEVER invoke interactive tools (vim, less, top) - they will hang. To FIND files, use the `glob` tool (fast, ignores node_modules/.git, capped) - a recursive shell scan (`Get-ChildItem -Recurse`, `find`, `dir /s`) from a large or home directory can time out. Match the shell in the <env> block: on Windows that is PowerShell, so use PowerShell syntax (`2>$null`, not `2>nul`). If cmd is specifically required for batch scripts or DOS commands, invoke it explicitly via `cmd /c ...`. Do NOT use `&&` to chain commands on Windows PowerShell; use `;` instead (e.g. `cd dir; cargo check`) because PowerShell 5.1 does not support `&&`. You are ALREADY inside a persistent PowerShell session on Windows: do NOT prefix commands with `powershell -Command \"...\"` or `powershell -NoProfile -Command \"...\". Run PowerShell commands directly (e.g. `Get-Content ...`).",
