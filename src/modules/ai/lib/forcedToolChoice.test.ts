...
import { MODELS, modelAllowsForcedToolChoice, modelKeepsReasoning } from "../config";
...
describe("a pinned tool choice is only sent where it is accepted", () => {
  ...
  it("is withheld from every reasoning-tagged model", () => {
    const reasoning = MODELS.filter((m) => m.tags?.includes("reasoning"));
  ...
  it("is still sent to models that take it", () => {
    const plain = MODELS.filter((m) => !m.tags?.includes("reasoning"));
