...
function extractStreamText(content: string): string {
  const out: string[] = [];
  // `Tj` shows one string. `TJ` shows an array of strings and kerning numbers;
  // extract every string inside the array (numbers are spacing, not text).
  const tjRe = /\(((?:\\.|[^\\()])*)\)\s*-?\d*\.?\d*\s*Tj/g;
...
function streamBlocks(
  ...
  for (const m of text.matchAll(re)) {
    ...
    // data is compressed. Read back a small window to check.
