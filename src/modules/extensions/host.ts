...
export async function buildContext(ext: ExtensionRuntime): Promise<{
  ...
  const readAiState = (): AiStateSnapshot => {
    ...
    let provider = "openai";
    try {
      provider = resolveModel(c.selectedModelId).provider;
    } catch {
      // Unknown model id: fall back to the default provider label.
    }
    return {
      ...
      usage: {
        input: c.agentMeta.tokens.inputTokens,
        output: c.agentMeta.tokens.outputTokens,
  ...
  const context: ExtensionContext = {
    ...
    ai: {
      ...
      async setModel(modelId: string, provider: string): Promise<void> {
        ...
        chatMod.useChatStore.getState().setSelectedModelId(
          String(known ? `${known}:${modelId}` : modelId ?? ""),
        );
      },
      ...
      async setSubagentsEnabled(enabled: boolean): Promise<void> {
        requirePermission(ext.id, declared, "ai:configure");
        // Termigo derives sub-agent availability from the tool set at run
        // time; the legacy toggle is accepted and ignored.
        void enabled;
      },
