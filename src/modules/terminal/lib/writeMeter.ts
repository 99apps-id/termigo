/**
 * Flow control for the xterm write path. Ported from TEDI's writeMeter.
 *
 ...
 * through the Tauri channel while the parser is effectively stopped: the backlog
 * grows unbounded behind the lock screen (hundreds of MB of retained chunks),
 * `Terminal.write` eventually throws at its own 50 MB ceiling — taking the rest
 * of the data handler with it — and unlocking pays for the whole backlog in one
 * parse storm. That is the "lock the PC, come back, the app is frozen" report.
 *
 * Past `WRITE_HIGH_WATER` bytes accepted-but-not-yet-parsed we stop feeding
 * xterm and hold the tail here instead, capped at `STALL_CAP`, resuming once the
 * parser has caught up to `WRITE_LOW_WATER`.
 */
 ...
 * dropped. Trimming a prefix would slice a CSI sequence in half and corrupt
 * xterm's screen state, so the whole held buffer goes and the screen is reset
 * instead of half-updated.
 */
export const STALL_NOTICE =
  "\x1bc\x1b[2m[termigo: dropped output while the window was hidden]\x1b[0m\r\n";
 ...
 *               slot has gone, so a late parser callback can't write into a dead
 *               terminal.
 */
