...
describe("shouldRecoverWebgl", () => {
  ...
  it("does not recover a slot bound to no leaf", () => {
    expect(shouldRecoverWebgl({ ...visible, currentLeafId: null }, true)).toBe(false);
  });
  ...
  it("does nothing when the addon is still attached", () => {
    expect(shouldRecoverWebgl({ ...visible, hasAddon: true }, true)).toBe(false);
  });
  ...
  it("honours the WebGL preference above everything else", () => {
    ...
    expect(shouldRecoverWebgl({ hasAddon: true, currentLeafId: null, parked: true }, false)).toBe(
      false,
    );
  });
  ...
  it("is false for a fresh idle slot", () => {
    expect(
      shouldRecoverWebgl({ hasAddon: false, currentLeafId: null, parked: false }, true),
    ).toBe(false);
  });
});
