...
const PLUGINS_BY_PARSER: Record<string, (() => Promise<Plugin>)[]> = {
  ...
  markdown: [
    () => import("prettier/plugins/markdown").then((m) => m.default ?? m),
  ],
  graphql: [
    () => import("prettier/plugins/graphql").then((m) => m.default ?? m),
  ],
};
...
function loadPluginsFor(parser: string): Promise<Plugin[]> {
  ...
  if (!loaders) {
    cached = Promise.reject(
      new Error(`prettier: no plugins registered for parser "${parser}"`),
    );
...
export async function formatWithPrettier(args: {
  ...
  const [prettier, plugins, editorConfig, projectConfig] = await Promise.all([
    ...
    args.filepath
      ? resolvePrettierConfig(args.filepath)
