...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    ...
    case "fedora":
      return `sudo dnf install ./Termigo-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `Termigo v${manual.version} is available`
                  : `Termigo v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart Termigo to finish installing."
              : downloading
                ? progress !== null
