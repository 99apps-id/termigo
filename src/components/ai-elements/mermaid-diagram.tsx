 ...
 * cytoscape, …), so keep it out of the initial bundle and only pull the chunk
 * when a diagram actually needs to render.
 */
...
/**
 * Renders a fenced ```mermaid block as an SVG diagram in AI chat output. The
 * source is untrusted, so mermaid runs with `securityLevel: "strict"` (labels
 * escaped, no scripts or click handlers) and the SVG it returns is already
 * sanitized, which is what makes the `dangerouslySetInnerHTML` below safe.
 */
export function MermaidDiagram({ code }: { code: string }) {
  ...
  useEffect(() => {
    // Don't parse a half-streamed diagram: it would just error and flash the
    // fallback until the block completes.
  ...
  }, [code, resolvedMode, streaming, id]);
